...
async function loadItemsFromCSV() {
    const response = await fetch("items_generated.txt");
    ...
    allItems = lines.slice(1).map(line => {
        const [name, theme, price, folder, description] = line.split(";");

        // Définit l'image à afficher dans le menu principal : menu.png ou menu.jpg
        let menuImage = `${folder}/menu.jpg`;
        // fallback sur menu.jpg si menu.png n'existe pas (simple, on suppose que les fichiers existent)
        // Tu peux vérifier l'existence côté serveur ou python si besoin
        // menuImage = menuImageExists ? menuImage : `${folder}/menu.jpg`;
...
function renderItems(items) {
    ...
    items.forEach(item => {
    const itemHTML = `
    <div class="item" data-folder="${item.folder}">
        <div class="itemIMGContainer">
            <img class="itemIMG" src="${item.menuImage}" onerror="this.src='${item.folder}/menu.png'" />
        </div>
    ...
    // Ajout des click events après génération
    container.querySelectorAll(".item").forEach(div => {
        div.addEventListener("click", () => {
            const name = div.querySelector(".title").textContent.trim();
                const safeName = name.replace(/\s+/g, "-");
                // Ouvre la page dans le même onglet au lieu d'un nouvel onglet
                window.location.href = `items/${safeName}.html`;
        });
    });
}
